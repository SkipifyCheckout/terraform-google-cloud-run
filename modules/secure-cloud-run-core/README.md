...
| node\_selector | GPU hardware requirements. Set the key `run.googleapis.com/accelerator` to the GPU type (e.g. `nvidia-l4`). Also add `"nvidia.com/gpu"` to `limits` and set `cpu_throttling = false`. [More info](https://cloud.google.com/run/docs/configuring/services/gpu). | `map(string)` | `null` | no |
